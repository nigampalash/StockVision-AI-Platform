...
export function Sidebar() {
  return (
    <aside className="w-64 flex-col hidden md:flex h-full border-r border-black/5 dark:border-white/5 glass-panel">
      <div className="p-6 flex items-center gap-3">
        <div className="w-8 h-8 rounded-lg bg-gradient-to-tr from-blue-600 to-indigo-500 flex items-center justify-center">
          <LineChart className="text-foreground dark:text-white w-5 h-5" />
        </div>
...
function NavItem({ href, icon, label, active = false }: { href: string; icon: React.ReactNode; label: string; active?: boolean }) {
  return (
    <Link href={href} className={`flex items-center gap-3 px-4 py-3 rounded-lg transition-all duration-200 ${active ? 'bg-primary/10 text-primary font-medium' : 'text-muted-foreground hover:bg-black/5 dark:bg-white/5 hover:text-foreground'}`}>
      {icon}
      <span>{label}</span>
    </Link>
  );
